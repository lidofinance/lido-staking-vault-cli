...
const getShareRateCacheFile = (vaultAddress: string) =>
  path.resolve('ipfs-cache', `share-rate-cache-${vaultAddress}.json`);
...
export const cache = {
  async getShareRate(
    ...
    try {
      const data = JSON.parse(
        await fs.readFile(getShareRateCacheFile(vaultAddress), 'utf-8'),
      );
  ...
  async setShareRate(vaultAddress: string, blockNumber: number, value: bigint) {
    ...
    await fs.mkdir(path.dirname(getShareRateCacheFile(vaultAddress)), {
      recursive: true,
    });
    await fs.writeFile(
      getShareRateCacheFile(vaultAddress),
      JSON.stringify(data),
      'utf-8',
    );
  },
