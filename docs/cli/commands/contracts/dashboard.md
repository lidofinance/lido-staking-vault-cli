...
| fee-leftover \<address>                               | get the amount of node operator fees accrued on the moment of disconnection and secured to be recovered to the `feeRecipient` address using `recoverFeeLeftover` method |
...
| connect-to-vault-hub connect-hub\<address>                                           | connects to VaultHub, transferring underlying StakingVault ownership to VaultHub.                                                                                                                                      |
...
| add-fee-exemption \<address> \<amount>                                               | adds a fee exemption to exclude this value from node operator fee base; increases settled growth as if fees were already paid on the exempted amount                                                                   |
| disburse-node-operator-fee \<address>                                                | transfers the node-operator`s accrued fee (if any) to nodeOperatorFeeRecipient                                                                                                                                         |
| change-tier ct\<address> \<tierId> \<requestedShareLimit> [--steth]                  | vault tier change with multi-role confirmation. `requestedShareLimit` is in shares by default; use `--steth` to provide it in stETH (converted on-chain via `getSharesByPooledEth`)                                    |
| sync-tier st\<address>                                                               | requests a sync of tier on the OperatorGrid                                                                                                                                                                            |
| update-share-limit usl\<address> \<shareLimit> [--steth]                             | requests a change of share limit on the OperatorGrid. `shareLimit` is in shares by default; use `--steth` to provide it in stETH (converted on-chain via `getSharesByPooledEth`)                                       |
...
| correct-settled-growth \<address> \<newSettledGrowth>                                | Manually corrects the settled growth value with dual confirmation.                                                                                                                                                     |
| recover-fee-leftover rfl\<address>                                                   | Recovers the previously collected fees to the feeRecipient address                                                                                                                                                     |
