...
/**
 * PDG describes the CL state layout differently once redeployed for Gloas
 * (lidofinance/core#1940): GI_FIRST_VALIDATOR_PREV/CURR become
 * GI_FIRST_VALIDATOR_PRE_GLOAS/GI_VALIDATORS and PIVOT_SLOT becomes GLOAS_SLOT.
 * Networks are upgraded at different times and one ABI serves them all, so read
 * whichever set the deployment actually exposes.
 */
export const readPdgClLayout = async (
  contract: PredepositGuaranteeContract,
) => {
  try {
    const [GI_FIRST_VALIDATOR_PRE_GLOAS, GI_VALIDATORS, GLOAS_SLOT] =
      await Promise.all([
        contract.read.GI_FIRST_VALIDATOR_PRE_GLOAS(),
        contract.read.GI_VALIDATORS(),
        contract.read.GLOAS_SLOT(),
      ]);
    return { GI_FIRST_VALIDATOR_PRE_GLOAS, GI_VALIDATORS, GLOAS_SLOT };
  } catch (err) {
  ...
  try {
    const [GI_FIRST_VALIDATOR_CURR, GI_FIRST_VALIDATOR_PREV, PIVOT_SLOT] =
      await Promise.all([
        contract.read.GI_FIRST_VALIDATOR_CURR(),
        contract.read.GI_FIRST_VALIDATOR_PREV(),
        contract.read.PIVOT_SLOT(),
      ]);
    return { GI_FIRST_VALIDATOR_CURR, GI_FIRST_VALIDATOR_PREV, PIVOT_SLOT };
  } catch (err) {
    if (!(err instanceof ContractFunctionExecutionError)) throw err;
    // Neither layout answered: the vendored ABI no longer matches the deployment
    throw new Error(
      `PredepositGuarantee at ${contract.address} exposes neither the Gloas CL layout getters ` +
        '(GI_FIRST_VALIDATOR_PRE_GLOAS/GI_VALIDATORS/GLOAS_SLOT) nor the pre-Gloas ones ' +
        '(GI_FIRST_VALIDATOR_CURR/PREV, PIVOT_SLOT). Check abi/PredepositGuarantee.ts against the deployed contract.',
      { cause: err },
    );
  }
};
